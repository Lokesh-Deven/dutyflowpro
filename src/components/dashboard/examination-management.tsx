              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField control={form.control} name="college" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g. Seshadripuram College" {...field} className="bg-white dark:bg-slate-800/50 border-primary dark:border-slate-700" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}/>
                <FormField control={form.control} name="examName" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="e.g. Annual Examination, March 2025" className="bg-white dark:bg-slate-800/50 border-primary dark:border-slate-700" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}/>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                                ...
                                <FormItem className="flex flex-col">
                                    ...
                                    <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
                                        <PopoverTrigger asChild>
                                            <FormControl>
                                                <Button variant={"outline"} className={cn("pl-3 text-left font-normal bg-white dark:bg-slate-800/50 border-primary dark:border-slate-700", !field.value && "text-muted-foreground")}>
                        ...
                        <div className="md:col-span-2">
                           ...
                           <Input 
                              ...
                              className="bg-white dark:bg-slate-800/50 border-primary dark:border-slate-700"
                           />
                        ...
                        <div>
                            <Label>No of Rooms</Label>
                             <Select value={sessionDetails.rooms.toString()} onValueChange={(value) => handleSessionDetailChange('rooms', parseInt(value, 10))}>
                                <SelectTrigger className="bg-white dark:bg-slate-800/50 border-primary dark:border-slate-700"><SelectValue/></SelectTrigger>
                        ...
                        <div className="md:col-span-2">
                           <Label>Time</Label>
                            <div className="grid grid-cols-3 gap-2">
                                <Select value={sessionDetails.startTimeHour} onValueChange={(v) => handleSessionDetailChange('startTimeHour', v)}><SelectTrigger className="bg-white border-primary dark:bg-slate-800/50 dark:border-slate-700"><SelectValue/></SelectTrigger><SelectContent>{hours.map(h => <SelectItem key={`st-h-${h}`} value={h}>{h}</SelectItem>)}</SelectContent></Select>
                                <Select value={sessionDetails.startTimeMinute} onValueChange={(v) => handleSessionDetailChange('startTimeMinute', v)}><SelectTrigger className="bg-white border-primary dark:bg-slate-800/50 dark:border-slate-700"><SelectValue/></SelectTrigger><SelectContent>{minutes.map(m => <SelectItem key={`st-m-${m}`} value={m}>{m}</SelectItem>)}</SelectContent></Select>
                                <Select value={sessionDetails.startTimePeriod} onValueChange={(v) => handleSessionDetailChange('startTimePeriod', v)}><SelectTrigger className="bg-white border-primary dark:bg-slate-800/50 dark:border-slate-700"><SelectValue/></SelectTrigger><SelectContent>{periods.map(p => <SelectItem key={`st-p-${p}`} value={p}>{p}</SelectItem>)}</SelectContent></Select>
                            </div>
                            <div className="grid grid-cols-3 gap-2 mt-2">
                                <Select value={sessionDetails.endTimeHour} onValueChange={(v) => handleSessionDetailChange('endTimeHour', v)}><SelectTrigger className="bg-white border-primary dark:bg-slate-800/50 dark:border-slate-700"><SelectValue/></SelectTrigger><SelectContent>{hours.map(h => <SelectItem key={`et-h-${h}`} value={h}>{h}</SelectItem>)}</SelectContent></Select>
                                <Select value={sessionDetails.endTimeMinute} onValueChange={(v) => handleSessionDetailChange('endTimeMinute', v)}><SelectTrigger className="bg-white border-primary dark:bg-slate-800/50 dark:border-slate-700"><SelectValue/></SelectTrigger><SelectContent>{minutes.map(m => <SelectItem key={`et-m-${m}`} value={m}>{m}</SelectItem>)}</SelectContent></Select>
                                <Select value={sessionDetails.endTimePeriod} onValueChange={(v) => handleSessionDetailChange('endTimePeriod', v)}><SelectTrigger className="bg-white border-primary dark:bg-slate-800/50 dark:border-slate-700"><SelectValue/></SelectTrigger><SelectContent>{periods.map(p => <SelectItem key={`et-p-${p}`} value={p}>{p}</SelectItem>)}</SelectContent></Select>
                            </div>
                        </div>
                    </div>
                  ...
                  {examinations.length === 0 ? (
                      ...
                      examinations.map((exam, index) => (
                          ...
                          >
                              ...
                              <TableCell className="text-slate-800 dark:text-slate-200">{exam.subject}</TableCell>
