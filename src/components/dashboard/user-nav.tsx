...
export function UserNav() {
  ...
  const singleInitial = (institutionName.trim().charAt(0) || userEmail.trim().charAt(0) || "U").toUpperCase();
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="relative h-9 w-9 rounded-full p-0 ring-2 ring-[#6342e8]/20 hover:ring-[#6342e8]/50 transition-all">
          <Avatar className="h-9 w-9">
            <AvatarFallback className="bg-[#6342e8] text-white font-black text-sm select-none">
              {singleInitial}
            </AvatarFallback>
          </Avatar>
        </Button>
      ...
      <DropdownMenuContent className="w-64 rounded-xl border-border shadow-md" align="end" forceMount>
        ...
        <DropdownMenuItem asChild className="cursor-pointer m-1 rounded-lg">
          <Link href="/dashboard/profile" className="flex items-center gap-2">
            ...
            <span className="font-medium text-xs">User Settings</span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
